...
echo "p'chour" | docker compose exec -T kafka opt/bitnami/kafka/bin/kafka-console-producer.sh --topic new_topic --bootstrap-server localhost:9092
...
# delete a topic:
docker compose exec kafka  opt/bitnami/kafka/bin/kafka-topics.sh --delete --topic trades_topic --bootstrap-server localhost:9092
# create a topic
docker compose exec kafka  opt/bitnami/kafka/bin/kafka-topics.sh \
    --create \
    --topic trades_topic \
